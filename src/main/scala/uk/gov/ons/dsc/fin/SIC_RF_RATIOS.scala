...
object SIC_RF_RATIOS {
  ...
  def  main(args:Array[String]):Unit = {
    ...
    println("Agruments format is: numFeatures StartPos endPos SIC_chars startCol")
      ...
      val numRatios =   try {args(5).toInt} catch {case e:Exception => println ("exception parcing number of ratio pairs: " + e); 1} //number of ratio pairs
      ...
      if (counter >= startPos && fCols != null && !fCols.isEmpty) {
        ...
        numRatios match {
          ...
          case 1 => {
            ...
            featureTransformer1.setInputCol2(fCols(1))
            assembler.setInputCols(Array("R1") ++ fCols.drop(2))
          }

          case 2 => {
            featureTransformer1.setInputCol1(fCols(0))
            featureTransformer1.setInputCol2(fCols(1))
            featureTransformer2.setInputCol1(fCols(2))
            featureTransformer2.setInputCol2(fCols(3))
            assembler.setInputCols(Array("R1","R2") ++ fCols.drop(4))
          }
          ...
          case 3 => {
            ...
            featureTransformer2.setInputCol1(fCols(2))
            featureTransformer2.setInputCol2(fCols(3))
            featureTransformer3.setInputCol1(fCols(4))
            featureTransformer3.setInputCol2(fCols(5))
            assembler.setInputCols(Array("R1","R2","R3") ++ fCols.drop(6))
          }
